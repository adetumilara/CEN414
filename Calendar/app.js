const date = new Date();
let myBirthday = new Date("2023-02-08")
let momBirthday = new Date("2023-05-21")
let dadBirthday = new Date("2023-05-10")
...
let workerDay = new Date("2023-05-01")
...
function renderCalendar () {
  ...
  for(let i=1; i<=lastDay; i++){
    ...
    else if(i=== workerDay.getDate() && date.getMonth()=== workerDay.getMonth()){
      days +=`<div class ='holiday'>${i}</div>`
    }
    else {
      days += `<div>${i}</div>`;
